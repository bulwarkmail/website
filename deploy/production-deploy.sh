#!/usr/bin/env bash
# Deploys a release of the website to https://bulwarkmail.org on mail.rath.li.
#
# Installed as /usr/local/bin/bulwarkmail-prod-deploy and run as the `ubuntu`
# user through a forced command on the deploy key in
# ~ubuntu/.ssh/authorized_keys:
#
#   command="/usr/local/bin/bulwarkmail-prod-deploy",restrict ssh-ed25519 AAAA... website production deploy
#
...
#
#   deploy <sha>   read a release tarball (built by
#                  .github/workflows/production.yml) from stdin, switch to it,
#                  check it, roll back on failure
#   rollback       switch back to the previous release
#   status         print the current release and the last few
#
# Layout under /opt/bulwarkmail-website-prod (owned by ubuntu):
#   releases/<time>-<sha>/  unpacked standalone builds, newest five kept
#   current -> releases/…   what pm2 runs (ecosystem.config.cjs)
# pm2 runs as root, so only the pm2 calls use sudo. Beta
# (/opt/bulwarkmail-website-beta, pm2 bulwarkmail-website-beta) is never
# touched. The old in-place checkout /opt/bulwarkmail-website is no longer
# served once the first release goes live.
set -euo pipefail

BASE=/opt/bulwarkmail-website-prod
APP=bulwarkmail-website
PORT=3010
KEEP_RELEASES=5
...
# What pm2 currently runs for $APP, empty if it does not know the app.
current_exec_path() {
  sudo -n pm2 jlist 2>/dev/null | node -e '
    let s = "";
    process.stdin.on("data", (d) => (s += d)).on("end", () => {
      let apps = [];
      try { apps = JSON.parse(s); } catch { }
...
run_current() {
  # The first release replaces the old `npm start` process, which pm2 cannot
  # reload into a different script path.
  if [[ "$(current_exec_path)" == "$BASE/current/server.js" ]]; then
    sudo -n pm2 reload "$BASE/ecosystem.config.cjs" --update-env
  else
    sudo -n pm2 delete "$APP" >/dev/null 2>&1 || true
...
case "$cmd" in
  deploy\ *)
    ...
    [[ -f "$rel/server.js" && -d "$rel/docs" && -f "$rel/deploy/ecosystem.production.config.cjs" ]] ||
      { log "not a release tarball"; rm -rf "$rel"; exit 3; }
    echo "$sha" > "$rel/REVISION"

    previous=$(readlink -f "$BASE/current" 2>/dev/null || true)
    cp "$rel/deploy/ecosystem.production.config.cjs" "$BASE/ecosystem.config.cjs"
    switch_to "$rel"
    run_current
    if healthy; then
