---
title: All features
description: The complete feature list of Bulwark Webmail, section by section.
order: 0
edition: both
---
...
This page lists everything Bulwark does, grouped by area, with links to the pages that explain each part. Bulwark Lite has every feature that runs in the browser. The ones that need the server (admin console, plugins, OAuth, settings sync, web push) are listed under [what Lite leaves out](/docs/getting-started/lite#what-is-off).

## Mail

Related pages: [Email](/docs/features/email), [Composing](/docs/features/email/composing), [Search](/docs/features/email/search), [Keyboard shortcuts](/docs/guides/keyboard-shortcuts).
...
- The Unified Mailbox combines Inbox, Sent, Drafts, Junk, Archive, and Trash. By default it stays inside the active account and its shared/group folders; an admin can unlock a cross-account mode that spans every connected account. The combined folder rows only appear once two or more accounts are connected, or "Include group inboxes" is on and a group inbox exists. With one account and no group inbox, turning the setting on adds the section header and nothing under it.
- All mail, Unread, and Starred obey that same account boundary and can be narrowed to a per-account folder selection. Every row names the folder its message came from. Each of the three is a separate admin policy gate (see [Admin & extensibility](#admin--extensibility)). If none is enabled on your instance, their toggles are missing from Settings → Appearance, and the Unified Mailbox section can look empty even with the feature on and several accounts connected. Ask your admin to enable the ones you need.
...
- Each tag can be shown always, only when it has unread mail, or never
...
Related pages: [Calendar](/docs/features/calendar).
...
Related pages: [Contacts](/docs/features/contacts).
...
Related pages: [Email](/docs/features/email), [Composing](/docs/features/email/composing).
...
Related pages: [Files](/docs/features/files).
...
Related pages: [Account security](/docs/guides/account-security), [S/MIME](/docs/guides/smime), [Authentication](/docs/getting-started/configuration/authentication), [Embedded SSO](/docs/guides/embedded-sso).
...
Related pages: [Customization](/docs/guides/customization), [Keyboard shortcuts](/docs/guides/keyboard-shortcuts).
...
Related pages: [Environment reference](/docs/getting-started/configuration/environment-reference).
...
Related pages: [Multiple accounts](/docs/guides/multi-account), [Stalwart setup](/docs/getting-started/configuration/stalwart-setup).
...
Related pages: [Admin dashboard](/docs/guides/admin), [Plugins](/docs/guides/plugins), [Marketplace](/docs/guides/marketplace), [Extensions](/docs/extensions/introduction).
...
Related pages: [Installation](/docs/getting-started/installation), [Docker](/docs/deployment/docker), [Bulwark Lite](/docs/getting-started/lite), [Install on Stalwart](/docs/deployment/stalwart-app), [Progressive Web App](/docs/features/pwa), [Anonymous usage stats](/docs/features/telemetry), [Updating](/docs/deployment/updating).
...
- Docker images on GHCR: `webmail` for releases, `webmail-beta` built from `main`, `webmail-dev` built from `dev`, and `webmail-lite` / `webmail-lite-beta` for Lite
...
- Bulwark Lite for Stalwart: `bulwark-lite-stalwart.zip` installs as a Stalwart `Application` (served by Stalwart itself under any prefix, on the same origin, with nothing to configure)
