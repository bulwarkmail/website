...
The aggregated numbers we derive from this data (active-instance counts, version distribution, feature adoption percentages, account-bucket histogram) are published on a read-only Grafana view at _(URL to be added when we ship)_, so anyone can audit what we have.
